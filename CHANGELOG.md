...
- **Last-known reading on stale** — When a reading becomes stale (> 6 minutes old), the bottom-right corner now displays `"Last: X.X"` showing the last known glucose value (mmol/L) before the device went offline. This provides context for how close the final reading was to in-range boundaries.
