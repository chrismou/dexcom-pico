...
- **Wi-Fi setup mode** (`run_wifi_setup`, `serve_setup`). The device raises a WPA2 AP and serves an HTML form. `wlan.scan()` is the only un-feedable blocking C call (1-3 s typically); the watchdog is fed immediately before and after. Do not move the scan inside the HTTP accept loop. STA and AP coexist during setup; after setup, `connect_wifi` reconnects the STA explicitly - do not rely on `rejoin_wifi` inside setup. Setup mode cannot run concurrently with the poll loop.
